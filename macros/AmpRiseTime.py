...
#################input parameters#######################
#run = "combine_1002_1015"
run = "combine_1016_1029"
#run = "1031"
...
cut = "ch1Amp>0.2 && ch1Amp<0.35 && ch2Amp > 0.005 && t1gausroot>0 && t2gausroot>100 && t2gausroot<120"

isUVlaser = True

deltaT_mean = 42.9 #for 407nm laser
RiseTime_mean = 0.67 # for 407nm laser
max_amp = 0.08 # for 407nm laser
max_timeReso = 0.02 # for 407nm laser
...
for ix in range(0, nbins_X):
	...
	hist_deltaT = ROOT.TH1F("hist_deltaT_"+str(x_this),"hist_deltaT_"+str(x_this),200,deltaT_mean-0.3,deltaT_mean+0.5)
	...
	highDeltaT=hist_deltaT.GetBinCenter(hist_deltaT.FindLastBinAbove(int(0.3*maximumY)))
	lowDeltaT=hist_deltaT.GetBinCenter(hist_deltaT.FindFirstBinAbove(int(0.3*maximumY)))
	tf1_gaus = ROOT.TF1("tf1_gaus","gaus", deltaT_mean-0.3, deltaT_mean+0.3)
	...
	hist_ch2Risetime = ROOT.TH1F("hist_ch2Risetime_"+str(x_this),"hist_ch2Risetime_"+str(x_this), 200, 0.0, 2.0)
	...
	tf1_gaus_risetime = ROOT.TF1("tf1_gaus_risetime","gaus", 0.0, 2.0)
...
gr_deltaT_vs_x.GetYaxis().SetRangeUser(deltaT_mean-0.2,deltaT_mean+0.2)
...
gr_ch2Risetime_vs_x.GetYaxis().SetRangeUser(RiseTime_mean-0.1, RiseTime_mean+0.1)
